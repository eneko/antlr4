...
open class LexerATNSimulator: ATNSimulator {
    ...
    public static var _match_calls = 0
    public static var match_calls: Int {
        get { mutex.synchronized { _match_calls }}
        set { mutex.synchronized { _match_calls = newValue }}
    }
    ...
    internal func getExistingTargetState(_ s: DFAState, _ t: Int) -> DFAState? {
        let edges = s.mutex.synchronized { s.edges }
        if edges == nil || t < LexerATNSimulator.MIN_DFA_EDGE || t > LexerATNSimulator.MAX_DFA_EDGE {
            return nil
        }

        let target = s.mutex.synchronized {
